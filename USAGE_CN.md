...
4.  **配置环境变量**:
    ...
    TTS_ENABLED=true # 设置为 false 可禁用文本转语音功能，默认为 true
    ```
...
5.  **运行机器人**:
    ...
    python main.py
    ```
    您应该会看到类似 `==> Connecting Telegram Client...` (根据 Telethon 的实际输出) 的输出，表示 Userbot 开始尝试登录。
...
-   **TTS 功能开关**:
    您可以通过在 `.env` 文件中设置 `TTS_ENABLED` 环境变量来控制是否启用文本转语音功能。
    -   `TTS_ENABLED=true`: 启用 TTS 功能（默认行为）。
    -   `TTS_ENABLED=false`: 禁用 TTS 功能。当禁用时，`!v` 命令将不会执行任何操作（或可能提示功能已禁用）。
    翻译功能 (`!fanyi`) 不受此开关影响，始终可用。

## 注意事项
-   **日志**: Userbot 会在控制台输出运行日志，包括处理的命令、API 请求状态以及可能发生的错误。检查日志有助于排查问题。
-   **TTS 队列**: TTS 请求会加入一个队列中进行处理，以避免并发问题（如果启用）。
